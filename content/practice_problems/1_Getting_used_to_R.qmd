...
includes all the integers in the given interval.
...
length shows the number of items in the list.

```{r}
max(x)
```

max shows the highest integer value in the given interval.

```{r}
x[x<10]
```

x\[x\<10\] gives all the integer values that are less than 10 in the
given interval.
...
x\^2 gives all the integer values in the list raised to the second
power.
...
x\[x \< 12 & x \> 7\] gives all the integers that are less than 12 but
greater than 7 in the given interval.
...
-1:2 gives the values between and including -1 and 2.

(-1):2 runs the same command as -1:2, gives the values between and
including -1 and 2.

-(1:2) is different from the two previous commands and applies what is
outside the parentheses to its inside.
...
Sleep has 62 rows and 12 columns. The variables show the body weight,
brain weight, total sleep, gestation, lifespan, predation, exposure, and
danger regarding 62 different animals. The variables also show data on
whether the animals are primates, non dreaming and dreaming states.
