...
hist(sampling_experiment, breaks = 0:25, xlab = "#of Right 
-eared people out of 25", ylab = "Probability of being drawn \n from population of p = 0.5", cex.main = 2, cex.axis = 1.5, cex.lab = 2)
```

```{r}
using_distriution = dbinom(0:25,25,.5)
using_distriution
sum(using_distriution)
Number_righteared = c(0:25)
pdf = data.frame(Number_righteared, using_distriution)
plot(0:25, using_distriution)
```
...
The null hypothesis regarding the proportion of right-eared people can
be rejected as all p-values fall under .05.
